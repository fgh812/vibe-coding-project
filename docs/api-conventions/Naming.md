...
> 프로젝트별 도메인 패키지 목록은 `context/project.md`에 정의한다.
...
│   └── {Domain}Service.java
├── dao/
│   ├── {Domain}QueryDAO.java           # MyBatis 복잡 조회
│   └── {Domain}CommandDAO.java         # MyBatis 복잡 변경 (필요시)
├── repository/
│   └── {Domain}Repository.java         # JPA 기본 CRUD
├── entity/
│   └── {Domain}.java
├── dto/
│   ├── {Domain}CreateDTO.java
│   ├── {Domain}UpdateDTO.java
│   └── {Domain}DetailDTO.java
└── vo/
    └── {Domain}{Meaning}VO.java
```
