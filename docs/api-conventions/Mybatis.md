...
## 개요
MyBatis XML 매퍼, DAO 인터페이스 작성 규칙을 정의한다.
MyBatis는 **복잡 조회(통계, 다중 조인, 동적 쿼리)**와 **벌크 처리**에 사용한다.
> 예시 코드는 패턴 설명용이며, 프로젝트별 도메인은 `context/project.md`를 참조한다.
...
resources/mapper/
├── {domain}/
│   ├── {Domain}Query.xml               # 조회 SQL
│   └── {Domain}Command.xml             # 변경 SQL (필요시)
└── ...
```

### DAO 인터페이스 위치
```
{base-package}.{domain}/dao/
